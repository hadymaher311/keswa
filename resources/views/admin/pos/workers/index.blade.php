        ...
        <div class="row">
            <div class="col-12">
                ...
                <div class="card-body">
                    ...
                    @endif
                    @can('delete pos_workers')
                        <form action="{{ route('workers.destroy') }}" method="POST" id="deleteForm">
                            @csrf
                            {{ method_field('DELETE') }}
                        ...
                        <tbody>
                            @foreach ($workers as $worker)
                                <tr>
                                    <td>
                                        <div class="custom-checkbox custom-control">
                                            <input name="workers[]" form="deleteForm" type="checkbox" data-checkboxes="mygroup" class="custom-control-input" value="{{ $worker->id }}" id="checkbox-{{ $loop->index+1 }}">
                                    ...
                                    <td>
                                        @can('update pos_workers')
                                            <form action="{{ route('workers.active', $worker->id) }}" method="post">
                                                @csrf
                                                @method('PUT')
                                                <label class="custom-switch mt-2" data-toggle="tooltip" data-placement="top" title="@if ($worker->active)
                                                    {{ __('Active') }}
                                                    @else
                                                    {{ __('Not Active') }}
                                                    @endif">
                                                    <input name="active" value="{{ $worker->id }}" type="checkbox" @if ($worker->active)
                                                    checked
                                    ...
                                    <td>
                                        @can('update pos_workers')
                                            <a href="{{ route('workers.edit', $worker->id) }}" data-toggle="tooltip" data-placement="top" title="{{ __('Edit') }}" class="btn btn-sm btn-warning">
                                                <i class="fa fa-edit"></i>
                                            </a>
                                        
                                            <a href="{{ route('workers.edit.password', $worker->id) }}" data-toggle="tooltip" data-placement="top" title="{{ __('Edit password') }}" class="btn btn-sm btn-dark">
                                            <i class="fa fa-lock"></i>
                                        </a>
                                        @endcan

                                        <a href="{{ route('workers.show', $worker->id) }}" data-toggle="tooltip" data-placement="top" title="{{ __('View') }}" class="btn btn-sm btn-primary">
                                        ...
                                        @can('delete pos_workers')
                                            ...
                                            <form action="{{ route('workers.destroy') }}" method="POST">
                                                @csrf
                                                {{ method_field('DELETE') }}
